...
export const ContractTxSuccessToast: FC<ContractTxResult> = ({ extrinsicHash, blockHash }) => {
  ...
  const explorerUrl = activeChain?.explorerUrls?.[SubstrateExplorer.PolkadotJs]
  const explorerDetailUrl = explorerUrl && blockHash ? `${explorerUrl}/query/${blockHash}` : null

  return (
    <div className="flex flex-col gap-0.5">
      <div>Transaction successful</div>
      {(subscanDetailUrl || explorerDetailUrl) && (
